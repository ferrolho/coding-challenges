...
def main():
    ...
    for i in range(200):
        ...
        collisionCheckingDone = False

        while not collisionCheckingDone:
            for p1 in particles:
                if sum(p1.position() == p2.position() for p2 in particles) > 1:
                    particles = [p for p in particles if p.position() != p1.position()]
                continue

            collisionCheckingDone = True
